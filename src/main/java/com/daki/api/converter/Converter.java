...
import com.daki.domain.model.dto.EnumPadraoDTO;
import com.daki.domain.model.dto.system.AbstractDto;
import com.daki.domain.model.enums.interfaces.IEnum;
import com.daki.domain.patterns.annotations.MappedFieldPropsChildren;
import com.daki.domain.patterns.annotations.OnlyField;
...
public class Converter<DTO extends AbstractDto<?>, Entity extends AbstractEntity<?>> {
    ...
    private static List<Field> getFieldsNotDeclared(Class classOrigem, List<Field> fieldsDestino, List<Field> fieldsOrigem) {
        ...
        if (ListUtil.isNotNullOrEmpty(fieldsDestino)) {
        ...
        if (!value.getClass().equals(field.getType())) {
            if (field.getType().isEnum() && value instanceof EnumPadraoDTO) {
                ...
                value = Stream.of(field.getType().getEnumConstants())
                        ...
                        .findFirst()
                        .orElseThrow(() -> new DomainException(EnumDomainException.ENUM_NOT_FOUND.getMessage(), padraoDTO.getKey()));
            }
        }
        return value;
    }
